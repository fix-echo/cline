...
### 使用流程
1.  编写一个 JSON 格式的测试规范文件，描述 gRPC 调用的输入和期望输出。
2.  运行 `ts-node runSpecs.ts <spec-file-or-folder>`。
3.  测试平台会启动，连接到本地的 Cline gRPC 服务器（默认端口 26040）。
4.  平台执行所有测试用例，并输出成功或失败信息。

此平台非常适合进行回归测试、性能基准测试和大规模自动化测试。

```mermaid
graph TB
A[JSON 测试规范] --> B[测试平台 index.ts]
B --> C[GrpcAdapter]
C --> D[Cline gRPC 服务器]
D --> C
C --> E[比较响应]
